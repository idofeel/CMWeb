...
void main() {
    ...
    vec3 u_LightLocation = vec3(0.0, 0.0, 1.0);
    vec3 u_LightDiffuse = vec3(0.85, 0.85, 0.85);
    vec3 u_LightAmbient = vec3(0.15, 0.15, 0.15);
    vec3 u_LightSpecular = vec3(0.85, 0.85, 0.85);

    vec3 vertexPosition= gl_Position.xyz;
    vec3 vectorPosition= (u_MVPMatrix * vec4(a_Vector+a_Position, 1.0)).xyz;
    vec3 normalVector = normalize(vectorPosition - vertexPosition);
    vec3 vectorLight = normalize(-u_LightLocation);
    float factorDiffuse = abs(dot(vectorLight, normalVector));
    factorDiffuse = clamp(factorDiffuse, 0.0, 1.0);
    v_Diffuse = vec4(u_LightDiffuse * factorDiffuse, 1.0);

    v_Ambient = vec4(u_LightAmbient, 1.0);

    vec3 eyeVector = normalize(vertexPosition - u_eyeLocation);
    vec3 reflection = normalize(2.0 * factorDiffuse * normalVector- vectorLight);
    float factorSpecular = abs(dot(reflection, eyeVector));
    factorSpecular = pow(factorSpecular, u_power);
    v_Specular = vec4(factorSpecular * u_LightSpecular, 1.0);
}
`;
...
void main() {
    vec3 DiffuseColor = (u_MaterialDiffuse * v_Diffuse).xyz * u_MaterialDiffuse.w;
    vec3 AmbientColor = (u_MaterialAmbient * v_Ambient).xyz * u_MaterialAmbient.w;
    vec3 SpecularColor = (u_MaterialSpecular * v_Specular).xyz * u_MaterialSpecular.w;
    vec3 EmissiveColor = u_MaterialEmissive.xyz;
    vec4 color;
    if (u_FragmentTex == 0) {
        color = vec4(DiffuseColor + AmbientColor + SpecularColor + EmissiveColor, u_MaterialEmissive.w);
    } else if (u_FragmentTex == 1) {
        vec3 TexColor = (texture2D(u_TextureUnit, v_TextureCoord)).xyz;
        color = vec4(TexColor * (DiffuseColor + AmbientColor + SpecularColor + EmissiveColor), u_MaterialEmissive.w);
    } else {
        color = vec4(u_PureColor.xyz, u_MaterialEmissive.w);
    }
    gl_FragColor = color;
}
`;
