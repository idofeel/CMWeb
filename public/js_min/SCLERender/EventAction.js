...
var objectIndex = -1;
var pickObjectIndexs = null;
var pickObjectVisible = false;
var pickObjectTransparent = 0.0;
var pickObjectMaterial = null;
var isMove = false;
var curDate = null;
var lastTime = 0;
var curTime = 0;
...
function phoneKeyDown(event) {
  ...
  if (event.targetTouches.length == 1) {
    ...
    (lastX1 = touch1.clientX - offsetLeft),
      (lastY1 = touch1.clientY - offsetTop);
    (lastX2 = touch2.clientX - offsetLeft),
      (lastY2 = touch2.clientY - offsetTop);
  }
}
function phoneKeyMove(event) {
  ...
  if (event.targetTouches.length == 1) {
    ...
    if (isMove) {
      if (objectIndex != -1) {
        if (
          Math.abs(x1 - lastX1) > moveSensitivity ||
          Math.abs(y1 - lastY1) > moveSensitivity
        ) {
          glRunTime.objectMove(
            objectIndex,
            2 * (x1 - lastX1),
            -2 * (y1 - lastY1)
          );
      ...
      if (
        Math.abs(x1 - lastX1) > rotateSensitivity ||
        Math.abs(y1 - lastY1) > rotateSensitivity
      ) {
    ...
    if (
      Math.abs(seta1 - seta2) <= zRotateSensitive &&
      Math.abs(mVec2 - mVec1) <= cameraMoveSensitivity
    ) {
      if (
        (Math.abs(x1 - lastX1) > moveSensitivity ||
          Math.abs(y1 - lastY1) > moveSensitivity) &&
      ...
      let dist1 = Math.pow(
        Math.pow(lastX1 - lastX2, 2) + Math.pow(lastY1 - lastY2, 2),
        0.5
      );
...
function phoneKeyUp(event) {
  ...
  if (isKeyDown && !isKeyMove && !isKeyRotate && !isKeyScale) {
    ...
    if (
      curTime - lastTime < doubleClickTimeMS &&
  ...
  if (isKeyTap) {
    ...
    if (isShiftDown && dragLeft) {
      if (isMove) {
        ...
        if (objectIndex > -1) {
          if (
            Math.abs(x - lastX) > moveSensitivity ||
            Math.abs(y - lastY) > moveSensitivity
          ) {
            glRunTime.objectMove(
              objectIndex,
              2 * (x - lastX),
