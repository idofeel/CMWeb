...
function addMouseListener(canvas){canvas.ontouchstart=function(event){isKeyTap=true;if(event.targetTouches.length==1){let touch=event.targetTouches[0];let x=touch.clientX-canvas.offsetLeft,y=touch.clientY-canvas.offsetTop;if(rect.left<=x&&x<rect.right&&rect.top<=y&&y<rect.bottom){}
lastX=x,lastY=y;}else if(event.targetTouches.length==2){let touch1=event.targetTouches[0],touch2=event.targetTouches[1];lastX1=touch1.clientX-canvas.offsetLeft,lastY1=touch1.clientY-canvas.offsetTop;lastX2=touch2.clientX-canvas.offsetLeft,lastY2=touch2.clientY-canvas.offsetTop;}}
canvas.ontouchmove=function(event){if(event.targetTouches.length==1){let touch=event.targetTouches[0];let x=touch.clientX-canvas.offsetLeft,y=touch.clientY-canvas.offsetTop;if(isPhone){if(isMove){if(objectIndex!=-1){if(Math.abs(x-lastX)>2||Math.abs(y-lastY)>2){glRunTime.objectMove(objectIndex,2*(x-lastX),-2*(y-lastY));lastX=x,lastY=y;return;}}}}
if(isKeyTap){if((Math.abs(x-lastX)>5&&Math.abs(x-lastX)<50)||(Math.abs(y-lastY1)>5&&Math.abs(y-lastY1)<50)){let factor=200.0/canvas.height;let degreeX=factor*(x-lastX);let degreeY=factor*(y-lastY);glRunTime.rotate(degreeX,degreeY,0);}}
lastX=x,lastY=y;}else if(event.targetTouches.length==2){let touch1=event.targetTouches[0],touch2=event.targetTouches[1];let x1=touch1.clientX-canvas.offsetLeft,y1=touch1.clientY-canvas.offsetTop;let x2=touch2.clientX-canvas.offsetLeft,y2=touch2.clientY-canvas.offsetTop;let vecX1=lastX1-lastX2,vecY1=lastY1-lastY2;let vecX2=x1-x2,vecY2=y1-y2;let mVec1=Math.sqrt(vecX1*vecX1+vecY1*vecY1);let mVec2=Math.sqrt(vecX2*vecX2+vecY2*vecY2);let seta1=Math.atan2(lastY2-lastY1,lastX2-lastX1);let seta2=Math.atan2(y2-y1,x2-x1);if(Math.abs(seta1-seta2)<=0.01&&Math.abs(mVec2-mVec1)<=10.0){if(Math.abs(x1-lastX1)>5||Math.abs(y1-lastY1)>5)
...
canvas.ontouchend=function(event){if(isKeyTap){let rect=event.target.getBoundingClientRect();if(event.targetTouches.length==1){if(isDigitalTwinMode){return;}
let touch=event.targetTouches[0];let x=touch.clientX-canvas.offsetLeft,y=touch.clientY-canvas.offsetTop;if(rect.left<=x&&x<rect.right&&rect.top<=y&&y<rect.bottom){objectIndex=glRunTime.pick(x,y,true,false);}}}
...
function animRun(){if(glRunTime.setCameraAnim(uCurFrame)){glRunTime.setObjectAnim(uCurFrame);getCurFrame(uCurFrame);uCurFrame++;animationClock=setTimeout("animRun()",uSleepTime);}else{animPause();animationStatus=ANIMEND;setAnmiIcon(true);}}
...
function setCurFrame(frame){if(frame>=0){uCurFrame=frame;if(glRunTime.setCameraAnim(uCurFrame)){glRunTime.setObjectAnim(uCurFrame);}
