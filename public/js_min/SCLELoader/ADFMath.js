let EPSILON=0.001;let ZERO=1e-6;let DZERO=1e-6;let DEPSILON=1e-3;let ADF_PI=3.141592654;function IKS_MAX(a,b){if(a>b)
	...
	function ADFMatrixIdentity(pM){if(pM==null)
	return false;pM.Clear();return true;}
	...
	ADFMATRIXToDoubleMATRIX(pM,g_arrTempM1);var dScaleSq=g_arrTempM1[0][0]*g_arrTempM1[0][0]+g_arrTempM1[0][1]*g_arrTempM1[0][1]+g_arrTempM1[0][2]*g_arrTempM1[0][2];if(dScaleSq<((1e-6)*(1e-6)))
	return false;var dInvSqScale=1.0/dScaleSq;for(var i=0;i<4;i++){for(var j=0;j<4;j++){g_arrTempMOut[i][j]=0.0;}}
	var i=0,j=0;for(j=0;j<3;j++)
	{for(i=0;i<3;i++)
	g_arrTempMOut.m[j][i]=g_arrTempM1[i][j]*dInvSqScale;g_arrTempMOut.m[j][3]=0.0;}
	for(i=0;i<3;i++)
	{g_vTempTrans[i]=0.0;for(j=0;j<3;j++)
	g_vTempTrans[i]-=g_arrTempM1[i][j]*g_arrTempM1[3][j]*dInvSqScale;g_arrTempMOut.m[3][i]=g_vTempTrans[i];}
	g_arrTempMOut.m[3][3]=1.0;DoubleMATRIXToADFMATRIX(g_arrTempMOut,pOut);return true;}
	function ADFMatrixScaling(pMScale,sx,sy,sz){if(pMScale==null)
	return;ADFMatrixIdentity(pMScale);pMScale._11=sx;pMScale._22=sy;pMScale._33=sz;}
	function ADFMatrixTranslation(pMTran,x,y,z){if(pMTran==null)
	return;ADFMatrixIdentity(pMTran);pMTran._41=x;pMTran._42=y;pMTran._43=z;}
	function ADFMatrixRotationAxis(pOut,pV,dAngle){if(pOut==null||pV==null)
	return;ADFMatrixIdentity(pOut);var dSin=Math.sin(dAngle);var dCos=Math.cos(dAngle);pOut._11=dCos+pV.x*pV.x*(1.0-dCos);pOut._12=pV.x*pV.y*(1.0-dCos)+pV.z*dSin;pOut._13=pV.x*pV.z*(1.0-dCos)-pV.y*dSin;pOut._21=pV.x*pV.y*(1.0-dCos)-pV.z*dSin;pOut._22=dCos+pV.y*pV.y*(1.0-dCos);pOut._23=pV.y*pV.z*(1.0-dCos)+pV.x*dSin;pOut._31=pV.x*pV.z*(1.0-dCos)+pV.y*dSin;pOut._32=pV.y*pV.z*(1.0-dCos)-pV.x*dSin;pOut._33=dCos+pV.z*pV.z*(1.0-dCos);}
	...
	function ISDNEAR(fValue1,fValue2){if((fValue1-fValue2)<DEPSILON&&(fValue1-fValue2)>DEPSILON*(-1))
	return true;return false;}
	function IsVecSameDir(v1,v2){g_vVec1.Copy(v1);ADFVec3Normalize(g_vVec1);g_vVec2.Copy(v2);ADFVec3Normalize(g_vVec2);if(ISDNEAR(g_vVec1.x,g_vVec2.x)&&ISDNEAR(g_vVec1.y,g_vVec2.y)&&ISDNEAR(g_vVec1.z,g_vVec2.z))
	return true;else
	{var dRadianZero=0.0035;var dLimitValue=0.9995;if((g_vVec1.x>dLimitValue&&g_vVec2.x>dLimitValue)||(g_vVec1.x<-dLimitValue&&g_vVec2.x<-dLimitValue)||(g_vVec1.y>dLimitValue&&g_vVec2.y>dLimitValue)||(g_vVec1.y<-dLimitValue&&g_vVec2.y<-dLimitValue)||(g_vVec1.z>dLimitValue&&g_vVec2.z>dLimitValue)||(g_vVec1.z<-dLimitValue&&g_vVec2.z<-dLimitValue))
	{var dRadian=ADFCalcArcCos(ADFVec3Dot(g_vVec1,g_vVec2));if(dRadian<dRadianZero)
	return true;}
	return false;}}
	function IsVecReverseDir(v1,v2){g_vVec1.Copy(v1);ADFVec3Normalize(g_vVec1);g_vVec2.Copy(v2);ADFVec3Normalize(g_vVec2);g_vVec2.Mul(-1);if(ISDNEAR(g_vVec1.x,g_vVec2.x)&&ISDNEAR(g_vVec1.y,g_vVec2.y)&&ISDNEAR(g_vVec1.z,g_vVec2.z))
	return true;else
	{var dRadianZero=0.0035;var dLimitValue=0.9995;if((g_vVec1.x>dLimitValue&&g_vVec2.x>dLimitValue)||(g_vVec1.x<-dLimitValue&&g_vVec2.x<-dLimitValue)||(g_vVec1.y>dLimitValue&&g_vVec2.y>dLimitValue)||(g_vVec1.y<-dLimitValue&&g_vVec2.y<-dLimitValue)||(g_vVec1.z>dLimitValue&&g_vVec2.z>dLimitValue)||(g_vVec1.z<-dLimitValue&&g_vVec2.z<-dLimitValue))
	{var dRadian=ADFCalcArcCos(ADFVec3Dot(g_vVec1,g_vVec2));if(dRadian<dRadianZero)
	return true;}
	return false;}}
	function GetCorrectSinCosValue(dCosValue){if(dCosValue>1.0+ZERO)
	{dCosValue=1.0;}
	...
	function ADFVec3TransformNormal(pOut,pV,pM){g_vBaseDouble4_1.x=pV.x;g_vBaseDouble4_1.y=pV.y;g_vBaseDouble4_1.z=pV.z;g_vBaseDouble4_1.w=0.0;ADFVec4Transform(g_vBaseDouble4_1,g_vBaseDouble4_1,pM);pOut.x=g_vBaseDouble4_1.x;pOut.y=g_vBaseDouble4_1.y;pOut.z=g_vBaseDouble4_1.z;}
	...
	function RotateAxisFromVecToVec(vAxis,v1,v2,arrRotValue){var bResult=true;g_vVec1.Copy(vAxis);ADFVec3Normalize(g_vVec1);g_vVec2.Copy(v1);ADFVec3Normalize(g_vVec2);g_vVec3.Copy(v2);ADFVec3Normalize(g_vVec3);var dSrcDot=ADFVec3Dot(g_vVec1,g_vVec2);var dTgtDot=ADFVec3Dot(g_vVec1,g_vVec3);var dEpsilon=1e-5;if(dSrcDot<-0.97||dSrcDot>0.97)
	dEpsilon=1e-6;else if(dSrcDot<-0.8||dSrcDot>0.8)
	dEpsilon=1e-5;else if(dSrcDot<-0.5||dSrcDot>0.5)
	dEpsilon=1e-4;else
	...
	if(tn<0.0)
	{tn=0.0;if(-d<0.0)
	sn=0.0;else if(-1*d>a)
	sn=sd;else
	{sn=-1*d;sd=a;}}
	else if(tn>td)
	{tn=td;if((-1*d+b)<0.0)
	sn=0.0;else if((-1*d+b)>a)
	sn=sd;else
	{sn=(-1*d+b);sd=a;}}
	var sc=0.0;var tc=0.0;if(ISDEQUALEX(sn,0.0,1e-32))
	sc=0.0;else
	sc=sn/sd;if(ISDEQUALEX(tn,0.0,1e-32))
	tc=0.0;else
	tc=tn/td;if(pNearestLinePt1!=null)
	{pNearestLinePt1.x=vLinePt11.x+sc*(vLinePt12.x-vLinePt11.x);pNearestLinePt1.y=vLinePt11.y+sc*(vLinePt12.y-vLinePt11.y);pNearestLinePt1.z=vLinePt11.z+sc*(vLinePt12.z-vLinePt11.z);}
	...
	return true;else if(ISDEQUALEX(CalcDistanceOfTwoLineSeg(vLinePt1,vLinePt2,vTriVert2,vTriVert3,pIntersectPt),0.0,1e-7))
	return true;else if(ISDEQUALEX(CalcDistanceOfTwoLineSeg(vLinePt1,vLinePt2,vTriVert3,vTriVert1,pIntersectPt),0.0,1e-7))
	return true;else
	return false;}}
	else
	...
	{if(pIntersectPt!=null)
	pIntersectPt.Copy(vLinePt1);return true;}
	else
	return false;}
	else if(bIsIsPointOnPlane2)
	{if(IsPointInTriangle(vLinePt2,vTriVert1,vTriVert2,vTriVert3))
	{if(pIntersectPt!=null)
	pIntersectPt.Copy(vLinePt2);return true;}
	else
	return false;}
	g_n1.Sub(vLinePt1,vTriVert1);g_n2.Sub(vLinePt2,vTriVert1);var dot1=ADFVec3Dot(g_n1,g_vVec3);var dot2=ADFVec3Dot(g_n2,g_vVec3);if(dot1*dot2>0.0)
	return false;g_vVec4.Sub(vLinePt2,vLinePt1);if(!CalcIntersectOfLinePlane(vLinePt1,g_vVec4,vTriVert1,g_vVec3,g_vVec5))
	return false;if(IsPointInTriangle(g_vVec5,vTriVert1,vTriVert2,vTriVert3))
	{if(pIntersectPt!=null)
	pIntersectPt.Copy(g_vVec5);return true;}
	else
	return false;}}